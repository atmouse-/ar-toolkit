...
if __name__ == "__main__":
    ...
    os.system("mount -o noatime,nodiratime /dev/disk/by-uuid/{} /mnt/udisk".format(UDISK_UUID))
    ...
    if _last_snapshot > _mixmax_snapshot:
        os.mkdir("/mnt/udisk/rootfs/{}".format(_last_snapshot))
        cmd = "btrfs send -p /mnt/root/{0} /mnt/root/{1} | gzip > /mnt/udisk/rootfs/{1}/{0}.gz".format(_mixmax_snapshot, _last_snapshot)
        print(cmd)
        os.system(cmd)
        os.system("sync")
        os.system("gzip -v -t /mnt/udisk/rootfs/{1}/{0}.gz".format(_mixmax_snapshot, _last_snapshot))
    else:
        print("nothing to do")
